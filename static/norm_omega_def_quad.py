...
gw = lambda u_p: g_w(u_p, popt[0], popt[1], popt[2])
...
plt.grid()
plt.legend()
plt.text(1300, 40, r'$a = {} \qquad b = {} \qquad c = {}$'.format(np.round(popt[0],8), np.round(popt[1],8), np.round(popt[2], 8)))
...
dgw = lambda u_p: d_g_w(u_p, popt[0], popt[1], popt[2])
...
plt.grid()
plt.legend()
plt.text(1500, 0.25, r'$R^2 = {}$'.format(np.round(R_D,2)))
